...
for fib_idx, fibD in enumerate(diameters):
    ...
    for idx,pw in enumerate(pws):
        cd_qs[fib_idx,idx, :] = np.loadtxt("inh_results/"+fibermodel+"/cd_thresholds_pw"+pw+"_"+"QS"+".txt", delimiter=",")[fib_idx,:]*1e3
        cd_ih[fib_idx,idx, :] = np.loadtxt("inh_results/"+fibermodel+"/cd_thresholds_pw"+pw+"_"+"IH"+".txt", delimiter=",")[fib_idx,:]*1e3

    # Strength-Duration data
    sd_explabel = "sopt" # "fix"
    for idx,dist in enumerate(distances_sd):
        sd_qs[fib_idx,idx, :] = np.loadtxt("inh_results/"+fibermodel+"/sd_thresholds_dist"+dist+"_"+"QS_"+sd_explabel+".txt", delimiter=",")[fib_idx,:]*1e3
        sd_ih[fib_idx,idx, :] = np.loadtxt("inh_results/"+fibermodel+"/sd_thresholds_dist"+dist+"_"+"IH_"+"bos"+".txt", delimiter=",")[fib_idx,:]*1e3
...
ax_errcd.set(xscale = "log", ylim = (0,25), ylabel = "Relative error (%)")
...
# ax[0].set_title("Current-distance")
# plt.show()
...
ax_errsd.set(xscale = "log", ylim = (0,60), ylabel = "Relative error (%)")
...
# ax[1].set_title("Strength-Duration")
plt.show()
